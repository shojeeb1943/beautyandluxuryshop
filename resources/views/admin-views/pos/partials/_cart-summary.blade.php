...
<div class="mb-3">
    <div class="text-dark fw-semibold mb-2">{{ translate('order_type') }}:</div>
    <ul class="list-unstyled d-flex flex-wrap gap-2 align-items-center pos-order-type-buttons mb-0">
        <li>
            ...
            <label for="pos_walk_in" class="btn btn-outline-dark btn-sm mb-0">{{ translate('walk_in') }}</label>
        </li>
        <li>
            <input type="radio" id="pos_delivery" class="pos-order-type" value="delivery" name="pos_order_type_ui" hidden>
            <label for="pos_delivery" class="btn btn-outline-dark btn-sm mb-0">{{ translate('home_delivery') }}</label>
        </li>
    </ul>
</div>

<div class="pos-delivery-section d-none mb-3 p-3 border rounded">
    <div class="text-dark fw-semibold mb-2">{{ translate('delivery_details') }}:</div>
...
@php($customerAddressesUrl = route('admin.pos.customer-addresses', ['customerId' => '__CID__']))
@php($shippingMethodsUrl = route('admin.pos.shipping-methods'))
<div class="d-none"
